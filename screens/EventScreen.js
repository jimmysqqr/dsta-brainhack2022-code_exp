...
const db = SQLite.openDatabase("events1.db"); //it will check for a database in the phone's file system. If it exist it will open it otherwise it will create it
...
export default function EventScreen({ navigation, route }) {
	...
	function deleteNote(id) {
		db.transaction(
			(tx) => {
				tx.executeSql(`DELETE FROM events1 WHERE id = ${id}`);
			},
			null,
			refreshEvents
		);
	...
	useEffect(() => {
		...
		if (route?.params) {
			...
			db.transaction(
				(tx) => {
					tx.executeSql(
						"INSERT INTO events1 (title, date, startTime, endTime, location, bring, attire, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
						[
							newNote.title,
							newNote.date,
							newNote.startTime,
